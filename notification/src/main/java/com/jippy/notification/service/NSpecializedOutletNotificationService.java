...
public class NSpecializedOutletNotificationService {
    ...
    public void consume(NOrderEvent event) {
        ...
        log.info("Received Kafka event orderId={} outletId={} status={}", event.getOrderId(), event.getOutletId(), event.getStatus());
        ...
        try {
            ...
            log.info("Notification processed successfully outletId={}", event.getOutletId());

        } catch (Exception ex) {

            log.error("Notification processing failed orderId={} outletId={}", event.getOrderId(), event.getOutletId(), ex);
        }
    }
    ...
    private void sendFCM(NOrderEvent event, Notification notification) {
        ...
        try {
            ...
            log.info("FCM sent successfully topic={} response={}", topic, response);

        } catch (FirebaseMessagingException ex) {

            log.error("FCM failed outletId={}", event.getOutletId(), ex);
