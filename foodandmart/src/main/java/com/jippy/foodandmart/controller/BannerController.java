...
public class BannerController {
    ...
    public ResponseEntity<List<CustomerBannerDto>> getActiveBanners(@RequestParam double lat, @RequestParam double lng) {
        ...
        GeoResults<RedisGeoCommands.GeoLocation<String>> geoResults = geoOps.search(GEO_KEY, GeoReference.fromCoordinate(lng, lat), new Distance(MAX_SEARCH_RADIUS, Metrics.KILOMETERS), args);
        ...
        // Extract found IDs
        List<String> nearbyIds = geoResults.getContent().stream().map(r -> r.getContent().getName()).collect(Collectors.toList());
        ...
        log.info("==========================rawMetadataList" + rawMetadataList);
