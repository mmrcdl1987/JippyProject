...
@Service
@Slf4j
public class DriverLocationRouteService {
    ...
    public void processDriverDeliveryHistory(DriverOrderEvent event) {
        ...
        if (rawCoords != null && rawCoords.size() > 1) {
            ...
            DriverOrder driverOrder = driverOrderRepository.findByDriverIdOrderId(event.getDriverId(),event.getOrderId()).
                    orElseThrow(() -> {
