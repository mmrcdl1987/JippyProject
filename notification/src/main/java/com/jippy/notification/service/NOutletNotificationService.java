...
public class NOutletNotificationService {
    ...
    public void consume(NOrderEvent event) {
        log.info("KAFKA_START | OUTLET_NOTIFICATION | orderId={} | notificationType={}",
                event != null ? event.getOrderId() : null, event != null ? event.getNotificationType() : null);

        try {
            if (event == null || event.getOutletId() == null || event.getOutletId() <= 0) {
                log.error("VALIDATION_FAILED | OUTLET_EVENT_INVALID");
                return;
            }
            ...
            log.info("KAFKA_END | OUTLET_NOTIFICATION_SENT | orderId={}", event.getOrderId());

        } catch (NotificationException ex) {
            log.error("KAFKA_ERROR | OUTLET_NOTIFICATION_FAILED | orderId={} | error={}",
