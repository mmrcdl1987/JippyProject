...
import com.jippy.customerandorder.feignClient.FoodManagementFeignClient;
...
@Slf4j
public class CoOrderRejectionServiceImpl
        implements CoOrderRejectionService {

    private final CoOrderRepository
            orderRepository;

    private final CoOrderRejectionRepository
            rejectionRepository;

    private final CoCustomerDeliveryAddressRepository
            addressRepository;

    private final FoodManagementFeignClient
            feignClient;

    private final KafkaTemplate<String, COOrderEvent>
            kafkaTemplate;

    @Override
    public CoOrderRejection rejectOrder(
            CoOrderRejectionRequestDto request) {

        log.info(
                "Reject order request received orderId={}",
                request.getOrderId()
        );
        ...
        if (alreadyRejected) {
            ...
            log.error(
                    "Order already rejected orderId={}",
                    request.getOrderId()
            );
         ...
         */
        if (COConstants.CUSTOMER.equalsIgnoreCase(
                request.getType())) {
            ...
            order.setOrderStatus(
                    COConstants.ORDER_STATUS_CANCELLED
            );
             ...
             */
            CoOrderRejection rejection =
                    CoOrderRejectionMapper
                            .toEntity(request);
         ...
         */
        order.setOrderStatus(
                COConstants.ORDER_STATUS_REJECTED
        );

        orderRepository.save(order);

        log.info(
                "Order status updated REJECTED orderId={}",
                order.getOrderId()
        );
         ...
         */
        CoCustomerDeliveryAddress address =
                addressRepository
                        .findById(
                                order.getCustomerDeliveryAddressId()
                        )
                        .orElseThrow(() ->
                                new CoBusinessException(
        ...
        if (outlets == null
            ...
            throw new CoBusinessException(
                    "No specialized outlets found"
            );
        }

        log.info(
                "Total specialized outlets found={}",
                outlets.size()
        );
        ...
        for (FmOutletDto outlet : outlets) {
            ...
            if (outlet.getOutletId()
                    .equals(request.getRejectedById())) {

                log.info(
                        "Skipping rejected outletId={}",
                        outlet.getOutletId()
                );
            ...
            log.info(
                    "Kafka event sent orderId={} outletId={}",
                    order.getOrderId(),
                    outlet.getOutletId()
            );
        }
        ...
        log.info(
                "Rejected order reassignment completed orderId={}",
                order.getOrderId()
        );
