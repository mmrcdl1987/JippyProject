...
public final class FmMasterProductMapper {
    ...
    public static void validateForCreate(FmMasterProductRequest req){
        ...
        validateVegNonVeg(req.getVeg(), req.getNonVeg());
    }
     ...
     * Veg = 0, NonVeg = 1
     */
    private static void validateVegNonVeg(Integer veg, Integer nonVeg) {
        ...
        if ((vegValue != 0 && vegValue != 1)
            ...
            throw new IllegalArgumentException("Veg and Non-Veg values must be either 0 or 1.");
        }

        // Exactly one should be selected
        if (vegValue + nonVegValue != 1) {
            throw new IllegalArgumentException("Please select either Veg or Non-Veg.");
        }
    }
    ...
    public static FmMasterProduct toEntity(FmMasterProductRequest dto) {
    ...
    public static void updateEntity(FmMasterProduct entity, FmMasterProductRequest dto) {

        Integer veg = dto.getVeg() != null ? dto.getVeg() : entity.getVeg();
        Integer nonVeg = dto.getNonVeg() != null ? dto.getNonVeg() : entity.getNonVeg();

        validateVegNonVeg(veg, nonVeg);
        ...
        if (dto.getNonVeg() != null)
            entity.setNonVeg(dto.getNonVeg());
        ...
        if (dto.getHasOptions() != null)
            entity.setHasOptions(dto.getHasOptions());

        if (dto.getOptionsEnabled() != null)
            entity.setOptionsEnabled(dto.getOptionsEnabled());
